  ...
  if (req.method === "POST") {
    try {
      ...
      if (!req.body.trackingid) {
        return res.status(400).json({ success: false, msg: "Missing 'tracking ID' in the request body." });
      }

      const trackingid = req.body.trackingid;

      // Find the card in the database based on the provided cardid
      const foundtracking = await Tracking.findOne({ TrackingID: trackingid });
      ...
      return res.status(200).json({ success: true, tracking: foundtracking });
    } catch (err) {
      console.error(err);
